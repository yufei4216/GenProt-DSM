...
class Config:
    ...
    # output
    out_dir: str = r"/home/yufei/20251210up/pred/fusion_dd"
    ...
    fused_d: int = 128   # fused representation dim after fusion projection (will be swept)
...
def main(cfg: Config = CFG):
    ...
    for d in D_SWEEP:
        ...
        run_cfg.branch_d = 128
        run_cfg.fused_d = int(d)
        run_cfg.out_dir = os.path.join(root_out, f"FUSED_{d}")

        print("\n" + "=" * 100)
        print(f"[SWEEP] protT5=bilstm (fixed) | GPN-MSA=gru (fixed) | fusion=gate | fused_d={d}")
