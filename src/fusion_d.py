...
class Config:
    ...
    # output
    out_dir: str = r"/home/yufei/20251210up/pred/fusion_d"
    ...
    d_model: int = 256
    fused_d: int = 128  # fusion/output dim (will be swept)
...
class HeadProj(nn.Module):
    """Project pooled d_model features to fused_d (also used as branch vector dim)."""
    def __init__(self, cfg: Config):
        super().__init__()
        d_in = int(cfg.d_model)
        d_out = int(cfg.fused_d)
...
class DualBranchFusionModel(nn.Module):
    ...
    Each branch returns a vector of dimension cfg.fused_d.
    ...
    def __init__(self, t5_enc: nn.Module, gpn_enc: nn.Module, cfg: Config):
        ...
        # ---- Gate A (scalar) ----
        self.gate_ln_g = nn.LayerNorm(d)
        self.gate_ln_t = nn.LayerNorm(d)

        self.gate_mlp = nn.Sequential(
            nn.Linear(2 * d, int(cfg.gate_hidden)),
            nn.GELU(),
            nn.Dropout(float(cfg.dropout)),
            nn.Linear(int(cfg.gate_hidden), 1),
        )
        # initialize last layer to produce alpha≈0.5 at start
        last = self.gate_mlp[-1]
        if isinstance(last, nn.Linear):
            nn.init.zeros_(last.weight)
            nn.init.zeros_(last.bias)

        self._gate_tau = float(getattr(cfg, "gate_tau_start", 4.0))

        # fused head
        self.fuse_ln = nn.LayerNorm(d)
        self.head_fused = nn.Linear(d, 1)

        # optional aux heads
        self.head_t5 = nn.Linear(d, 1) if float(getattr(cfg, "aux_t5_w", 0.0)) > 0 else None
        self.head_gpn = nn.Linear(d, 1) if float(getattr(cfg, "aux_gpn_w", 0.0)) > 0 else None
...
def _run_single(cfg: Config = CFG):
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(n_train), y_train), start=1):
        ...
        export_fused_to_memmap(
            c_model,
            DataLoader(ds_tr, batch_size=32, shuffle=False, num_workers=0),
            device,
            fused_tr_path,
            n_rows=len(tr_idx),
            cfg=cfg,
        ...
        export_fused_to_memmap(
            c_model,
            DataLoader(ds_va, batch_size=32, shuffle=False, num_workers=0),
            device,
            fused_va_path,
            n_rows=len(va_idx),
            cfg=cfg,
        ...
        export_fused_to_memmap(
            c_model,
            DataLoader(ds_te, batch_size=32, shuffle=False, num_workers=0),
            device,
            fused_te_path,
            n_rows=n_test,
            cfg=cfg,
...
def main(cfg: Config = CFG):
    ...
    for d in D_SWEEP:
        ...
        run_cfg.fusion_mode = "gate"
        run_cfg.d_model = int(d)
        run_cfg.fused_d = int(d)
        run_cfg.out_dir = os.path.join(root_out, f"DIM_{d}")

        print("\n" + "=" * 100)
        print(f"[SWEEP] protT5=bilstm (fixed) | GPN-MSA=gru (fixed) | fusion=gate | d={d}")
        print(f"[OUT]   {run_cfg.out_dir}")
        print("=" * 100 + "\n")

        try:
            _run_single(run_cfg)
            all_results[str(d)] = {"ok": True}
        except Exception as e:
            all_results[str(d)] = {"ok": False, "error": repr(e)}
            print(f"[Error] d={d} failed: {e}")

    with open(os.path.join(root_out, "dim_sweep_summary.json"), "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)

    print("\n" + "=" * 100)
    print(f"[Sweep Finished] saved: {os.path.join(root_out, 'dim_sweep_summary.json')}")
    print("=" * 100)
