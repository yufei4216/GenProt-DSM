...
class Config:
    ...
    # output
    out_dir: str = r"/home/yufei/20251210up/pred/fusion"
    ...
    t5_backbone: str = "bigru"
    gpn_backbone: str = "bilstm"
...
class Scheme2PairEncoder(nn.Module):
    ...
    - forward(a_raw, b_raw) returns pooled 128-d vector via global mean pooling -> Head128
    """
...
class DualBranchFusionModel(nn.Module):
    """Two-branch encoder + fusion (no gate).

    Fixed encoders (controlled by cfg.t5_backbone / cfg.gpn_backbone) and selectable fusion strategies:
      - avg:        fused = (g + t) / 2
      - concat:     fused = Linear([g; t]) -> 128
      - bilinear:   fused = W((Wg g) ⊙ (Wt t)) -> 128   (low-rank factorized bilinear)
      - xattn:      token-level cross-attention between branches, then pool -> 128, then avg

    Returns:
      fused (B,128), logits_fused (B,1), optional aux logits
    """
    ...
    def __init__(self, t5_enc: nn.Module, gpn_enc: nn.Module, cfg: Config):
        ...
        self.fusion_mode = str(getattr(cfg, 'fusion_mode', 'avg')).lower()

        # concat fusion
        self.concat_proj = nn.Linear(256, 128) if self.fusion_mode == 'concat' else None

        # bilinear fusion (factorized)
        if self.fusion_mode == 'bilinear':
            r = int(getattr(cfg, 'bilinear_rank', 256))
            self.bilin_g = nn.Linear(128, r)
            self.bilin_t = nn.Linear(128, r)
            self.bilin_out = nn.Linear(r, 128)
            self.bilin_drop = nn.Dropout(float(cfg.dropout))
        else:
            self.bilin_g = self.bilin_t = self.bilin_out = self.bilin_drop = None

        # cross-attention fusion (token-level)
        if self.fusion_mode == 'xattn':
            d = int(cfg.d_model)
            h = int(getattr(cfg, 'xattn_heads', 8))
            self.xattn_t = nn.MultiheadAttention(d, h, dropout=float(cfg.dropout), batch_first=True)
            self.xattn_g = nn.MultiheadAttention(d, h, dropout=float(cfg.dropout), batch_first=True)
            self.xattn_ln_t = nn.LayerNorm(d)
            self.xattn_ln_g = nn.LayerNorm(d)
        else:
            self.xattn_t = self.xattn_g = None
            self.xattn_ln_t = self.xattn_ln_g = None

        self.fuse_ln = nn.LayerNorm(128)
        self.head_fused = nn.Linear(128, 1)

        self.aux_t5_w = float(cfg.aux_t5_w)
        self.aux_gpn_w = float(cfg.aux_gpn_w)
        self.head_t5 = nn.Linear(128, 1) if self.aux_t5_w > 0 else None
        self.head_gpn = nn.Linear(128, 1) if self.aux_gpn_w > 0 else None

    def _fuse_vec(self, g_in: torch.Tensor, t_in: torch.Tensor) -> torch.Tensor:
        m = self.fusion_mode
        if m == 'avg':
            return 0.5 * (g_in + t_in)
        if m == 'concat':
            return self.concat_proj(torch.cat([g_in, t_in], dim=1))
        if m == 'bilinear':
            # low-rank bilinear: (Wg g) ⊙ (Wt t) -> Wout
            hg = F.gelu(self.bilin_g(g_in))
            ht = F.gelu(self.bilin_t(t_in))
            h = self.bilin_drop(hg * ht)
            return self.bilin_out(h)
        raise ValueError(f"Unknown fusion_mode={m}")

    def forward(self, t5_wt, t5_mut, gpn_ref, gpn_alt):
        # (1) get branch vectors
        if self.fusion_mode == 'xattn':
            # token-level interaction; requires Scheme2PairEncoder.encode_tokens
            t_tok = self.t5_enc.encode_tokens(t5_wt, t5_mut)   # (B,2L,d_model)
            g_tok = self.gpn_enc.encode_tokens(gpn_ref, gpn_alt)  # (B,2L,d_model)

            # cross attention in both directions
            t_ctx, _ = self.xattn_t(t_tok, g_tok, g_tok)
            g_ctx, _ = self.xattn_g(g_tok, t_tok, t_tok)
            t_ctx = self.xattn_ln_t(t_ctx)
            g_ctx = self.xattn_ln_g(g_ctx)

            # pool to d_model
            t_pool = t_ctx.mean(dim=1)
            g_pool = g_ctx.mean(dim=1)

            # map to 128 using each branch head
            t5_128 = self.t5_enc.head(t_pool)
            gpn_128 = self.gpn_enc.head(g_pool)

            g_in, t_in = gpn_128, t5_128
            if self.moddrop is not None:
                g_in, t_in = self.moddrop(g_in, t_in)

            # for xattn fusion, use avg after interaction
            fused = 0.5 * (g_in + t_in)
        else:
            t5_128 = self.t5_enc(t5_wt, t5_mut)
            gpn_128 = self.gpn_enc(gpn_ref, gpn_alt)

            g_in, t_in = gpn_128, t5_128
            if self.moddrop is not None:
                g_in, t_in = self.moddrop(g_in, t_in)

            fused = self._fuse_vec(g_in, t_in)

        fused = self.fuse_ln(fused)
        logits_fused = self.head_fused(fused)

        lt5 = self.head_t5(t5_128) if self.head_t5 is not None else None
        lgpn = self.head_gpn(gpn_128) if self.head_gpn is not None else None
...
@torch.no_grad()
def export_fused_to_memmap(model: nn.Module, dl: DataLoader, device: torch.device, out_npy: str, n_rows: int):
    model.eval()
    ensure_dir(os.path.dirname(out_npy))

    mm = np.lib.format.open_memmap(out_npy, mode="w+", dtype=np.float32, shape=(n_rows, 128))
...
def train_d_complex(X_tr: np.ndarray, y_tr: np.ndarray, X_va: np.ndarray, y_va: np.ndarray,
    ...
    net = Classifier2L(cfg.d_hidden1, cfg.d_hidden2, cfg.d_dropout).to(device)
...
def _run_single(cfg: Config = CFG):
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(n_train), y_train), start=1):
        ...
        export_fused_to_memmap(c_model, DataLoader(ds_tr, batch_size=32, shuffle=False, num_workers=0), device, fused_tr_path, n_rows=len(tr_idx))
        export_fused_to_memmap(c_model, DataLoader(ds_va, batch_size=32, shuffle=False, num_workers=0), device, fused_va_path, n_rows=len(va_idx))
        export_fused_to_memmap(c_model, DataLoader(ds_te, batch_size=32, shuffle=False, num_workers=0), device, fused_te_path, n_rows=n_test)
...
# =========================
# Sweep runner (fusion strategy ablation; fixed encoders)
# =========================
FUSION_SWEEP = [
    "concat",     # concat + linear projection
    "bilinear",   # factorized bilinear
    "xattn",      # cross-modal attention
    "avg",        # average fusion
]


def main(cfg: Config = CFG):
    # Run the full pipeline sequentially for each fusion strategy.
    # Fixed encoders: protT5=BiLSTM, GPN-MSA=GRU.

    if cfg.do_memmap_convert:
        print("[Info] do_memmap_convert=True -> converting pth to memmap once (shared for all runs)")
        prepare_memmaps(cfg)
    ...
    for mode in FUSION_SWEEP:
        run_cfg = copy.deepcopy(cfg)
        run_cfg.t5_backbone = "bilstm"
        run_cfg.gpn_backbone = "gru"
        run_cfg.fusion_mode = mode
        run_cfg.out_dir = os.path.join(root_out, f"FUSION_{mode}")

        print("\n" + "=" * 100)
        print(f"[SWEEP] protT5=bilstm (fixed) | GPN-MSA=gru (fixed) | fusion={mode}")
        print(f"[OUT]   {run_cfg.out_dir}")
        print("=" * 100 + "\n")

        try:
            _run_single(run_cfg)
            all_results[mode] = {"ok": True}
        except Exception as e:
            all_results[mode] = {"ok": False, "error": repr(e)}
            print(f"[Error] fusion={mode} failed: {e}")

    with open(os.path.join(root_out, "sweep_summary.json"), "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)

    print("\n" + "=" * 100)
    print(f"[Sweep Finished] saved: {os.path.join(root_out, 'sweep_summary.json')}")
    print("=" * 100)
