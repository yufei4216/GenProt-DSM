...
class Config:
    ...
    # output
    out_dir: str = r"/home/yufei/20251210up/pred/GPNMSA_enhanced"
...
class Scheme2PairEncoder(nn.Module):
    """
    Scheme2: pseudo-site + relpos + site-marker, then a backbone encoder, then global mean pooling -> 128.
    """
...
class DualBranchFusionModel(nn.Module):
    """Two-branch encoder + fusion.

    Gate A (baseline): scalar MLP gate with temperature (tau) scheduling.
      alpha = sigmoid( MLP([LN(g), LN(t)]) / tau )
      fused = alpha * g + (1 - alpha) * t

    Notes:
      - We compute the gate on LayerNorm'ed features for scale stability, but fuse the original
        representations to preserve information.
      - The last MLP layer is initialized to zeros so alpha starts near 0.5 (i.e., average fusion).
    """
    ...
    def __init__(self, t5_enc: nn.Module, gpn_enc: nn.Module, cfg: Config):
        ...
        # gate inputs normalization
        self.gate_ln_g = nn.LayerNorm(128)
        self.gate_ln_t = nn.LayerNorm(128)

        # scalar MLP gate: 256 -> hidden -> 1
        h = int(getattr(cfg, 'gate_hidden', 64))
        ...
        # temperature (tau) used in forward; updated by tau schedule in train_c
        self.gate_tau = float(getattr(cfg, 'gate_tau_start', 4.0))

        # initialize so alpha ~= 0.5 at start
    ...
    def set_gate_tau(self, tau: float):
        # allow the training loop to update tau each epoch
    ...
    def forward(self, t5_wt, t5_mut, gpn_ref, gpn_alt):
        ...
        xcat = torch.cat([g_gate, t_gate], dim=1)  # (B,256)

        tau = max(float(self.gate_tau), 1e-6)
        alpha = torch.sigmoid(self.gate_mlp(xcat) / tau)  # (B,1)

        fused = alpha * g_in + (1.0 - alpha) * t_in
...
# =========================
# Sweep runner (GPN-MSA backbone ablation; protT5 fixed to BiLSTM)
# =========================
GPN_MSA_BACKBONE_SWEEP: List[str] = [
    "transformer",
    "tcn",
    "bilstm",
    "cnn",
...
def main(cfg: Config = CFG):
    """
    Run the full pipeline sequentially for each GPN-MSA module choice.
    - protT5 branch backbone: fixed to "bilstm" (selected as best in the protT5_enhanced sweep)
    - GPN-MSA branch backbone: swept over GPN_MSA_BACKBONE_SWEEP
    Outputs are written under: cfg.out_dir / GPNMSA_<backbone>/
    """
    ...
    for b in GPN_MSA_BACKBONE_SWEEP:
        run_cfg = copy.deepcopy(cfg)
        # fixed best protT5 encoder
        run_cfg.t5_backbone = "bilstm"
        # sweep GPN-MSA encoder
        run_cfg.gpn_backbone = b
        run_cfg.out_dir = os.path.join(root_out, f"GPNMSA_{b}")

        print("\n" + "=" * 100)
        print(f"[SWEEP] protT5 backbone = bilstm (fixed) | GPN-MSA backbone = {b}")
        print(f"[OUT]   {run_cfg.out_dir}")
        print("=" * 100 + "\n")

        try:
            _run_single(run_cfg)
            all_results[b] = {"ok": True}
        except Exception as e:
            all_results[b] = {"ok": False, "error": repr(e)}
            print(f"[Error] backbone={b} failed: {e}")
